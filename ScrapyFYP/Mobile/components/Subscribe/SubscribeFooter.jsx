...
const SubscribeFooter = ({form, setForm, category, brand, model}) => {
    ...
    const {user, spiders, condition, subscribeParams, setSubscribeParams, jwtToken} = useGlobalContext()
    ...
    const sendUserSubscribeReqeust = async(body) => {
      try{
        const data = await normalApiCall("UserSubscription/AddUserSubscription","POST",body,{jwtToken: jwtToken})
    ...
    const searchHandler = ()=>{
      ...
      setSubscribeParams({...subscribeParams, lowest_price:low, highest_price: high, spider: platform, condition:pcondition, description:description})
  ...
  return (
    <View>
      ...
      <View>
        <Text className = "text-3xl font-bold italic text-center mt-3">Choose Notification Type</Text>
        <View className ="flex flex-row justify-around">
            <CustomSelector 
                title = "email"
                customStyle = "w-32"
      ...
      <Text className = "text-3xl font-bold italic text-center mt-3">Choose Notification Time</Text>
      <FormField
